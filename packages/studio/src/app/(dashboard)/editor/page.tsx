...
function EditorInner() {
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      {sidebarOpen && (
        <aside className="w-64 border-r border-border flex-shrink-0 flex flex-col bg-background">
          <div className="p-4 border-b border-border">
            <Link href={`/book?id=${bookId}`} className="font-serif tracking-tight text-lg hover:underline block truncate">
              {book?.title || '未命名'}
            </Link>
          </div>
          <nav className="flex-1 overflow-y-auto py-2">
            {chapters.length > 0 ? (
              chapters.map((ch: any) => {
                const isActive = ch.num === num;
                return (
                  <Link
                    key={ch.num}
                    href={`/editor?bookId=${bookId}&num=${ch.num}`}
                  ...
                  >
                    <span className="text-xs text-muted/60 mr-2">{ch.num}</span>
                    {ch.title || `第${ch.num}章`}
                  </Link>
                );
              })
            ) : (
              <p className="px-4 py-3 text-xs text-muted">暂无章节</p>
            )}
          </nav>
          <div className="p-4 border-t border-border">
            <span className="text-xs text-muted/40">CoNovel v0.1.0</span>
          </div>
        </aside>
      )}
        ...
        <header className="border-b border-border px-4 py-2 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            <button
              ...
              className="text-muted hover:text-foreground text-xs px-1 transition-colors"
