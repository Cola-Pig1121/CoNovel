...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const s = rj(sf(id), { foreshadowing: [] });
  return NextResponse.json({ foreshadowing: s.foreshadowing || [] });
}
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const s = rj(f, { foreshadowing: [] } as any);
  const body = await req.json();
  const item = { id: crypto.randomUUID(), status: 'planted', ...body };
  s.foreshadowing = s.foreshadowing || [];
  s.foreshadowing.push(item);
